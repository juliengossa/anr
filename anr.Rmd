...
anr.col <- anr %>%
  ...
  bind_rows(
    anr %>%
      filter(type == "DOS") %>%
      group_by(sexe = sexe.partenaires,type=paste("Partenariat",type)) %>%
      summarise(
        Nombre.projets = n(),
        Financement = sum(Projet.Aide_allouee,na.rm = TRUE)
        )
...
anr.taux %>%
  ...
  ggplot(aes(x=édition,y=value,color=name)) + geom_line(size = 1) + geom_point(shape = 21, size=3, stroke= 1, fill="white") +
  expand_limits(y=0) +
  ...
  scale_color_brewer(palette = "Dark2", name="Part de")
```
